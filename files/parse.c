...
/*   Updated: 2021/03/25 09:49:35 by liafigli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int check_type(t_global *a, char **words)
{
    ...
    if (*words[0] == 'l')
        parse_light(a, words);
    /*if (ft_strncmp(words[0], "sp", ft_strlen(words[0])))
        parse_sphere(a, words);
    if (ft_strncmp(words[0], "sq", ft_strlen(words[0])))
        parse_square(a, words);
    if (ft_strncmp(words[0], "tr", ft_strlen(words[0])))
        parse_triangle(a, words);
    if (ft_strncmp(words[0], "cy", ft_strlen(words[0])))
        parse_cylinder(a, words);
    if (ft_strncmp(words[0], "pl", ft_strlen(words[0])))
        parse_plane(a, words);*/
    return (0);
}
