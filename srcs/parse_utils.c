...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/04/08 14:15:49 by liafigli          #+#    #+#             */
/*   Updated: 2021/04/08 14:37:28 by liafigli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (ft_strncmp(rt->split[0], "R", 1 == 0) && !rt->res.parsed)
		parse_res(rt);
	else if (ft_strncmp(rt->split[0], "A", 1 == 0) && !rt->ambient.parsed)
		parse_ambient(rt);
	else if (ft_strncmp(rt->split[0], "c", 1 == 0))
		parse_camera(rt);
	else if (ft_strncmp(rt->split[0], "l", 1 == 0))
		parse_light(rt);
	else if (ft_strncmp(rt->split[0], "sp", 2 == 0))
		parse_sphere(rt);
	else if (ft_strncmp(rt->split[0], "pl", 2 == 0))
		parse_plane(rt);
	else if (ft_strncmp(rt->split[0], "sq", 2 == 0))
		parse_square(rt);
	else if (ft_strncmp(rt->split[0], "cy", 2 == 0))
		parse_cylindre(rt);
	else if (ft_strncmp(rt->split[0], "tr", 2 == 0))
		parse_triangle(rt);
	else if (rt->split[0][0] == '#' || ft_strncmp(rt->split[0], "#", 1== 0))
		;
...
int			check_split(char **split, int max)
{
	...
	while (split[i])
	{
		...
		while (split[i][j])
		{
			if (i == 3 && ft_strncmp(split[0], "sp", 2 == 0)
				&& ft_strncmp_last(split[i], ".xpm", 3 == 0))
				break ;
